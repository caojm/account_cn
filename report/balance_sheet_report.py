...
class SubsidiaryLedgerReport(models.AbstractModel):
    ...
    def _create_balance_sheet(self, data, report_data):
            ...
            for item in report_model.item_ids:
                if item.input:
                    ...
                    for f in item.filter_ids:
                        data_source_filtered = filter(
                            lambda r: r["account_id"][0] == f.account_id.id, data_source
                        )
                        if f.reclassify:
                            if f.amount_type == "balance":
                                if f.orientation == "debit":
                                    balance_data = sum(
                                        [
                                            x["debit"] - x["credit"]
                                            for x in data_source_filtered
                                            if x["debit"] - x["credit"] > 0.0
                                        ]
                                    )
                            ...
                            if f.amount_type == "balance":
                                ...
                                if f.orientation == "debit":
                                    movement_data = sum(
                                        [x["debit"] for x in data_source_filtered]
                                    )
                                    if f.aggregation == "plus":
                                        bs_data[item.code] += movement_data
                                    else:
                                    ...
                                    movement_data = sum(
                                        [x["credit"] for x in data_source_filtered]
                                    )
                                    if f.aggregation == "plus":
                                        bs_data[item.code] += movement_data
                                    else:
